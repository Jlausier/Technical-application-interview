...
function App() {
  ...
  const fetchRandomDrinks = async () => {
    try {
      ...
      setLoading(true);
      const response = await axios.get('https://ec2-13-59-85-100.us-east-2.compute.amazonaws.com:8000/api/random_drink/');
  ...
  const fetchBreweriesByCity = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`https://ec2-13-59-85-100.us-east-2.compute.amazonaws.com:8000/api/search_nearby_breweries/${city}/`);
